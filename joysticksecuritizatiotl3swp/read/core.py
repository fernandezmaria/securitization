...
class EconRegltyCapital:
    ...
    def __init__(
        ...
        self.rtg_score_letter_rel = Ratings(
            "/data", self.spark, self.dataproc
        ).get_score_letter_relations()
        ...
        self.econ_capital_sel = econ_capital_sel.select(
            *[
                F.when(F.col(c) == 999999999999999.99, F.lit(0))
                .otherwise(F.col(c))
                .alias(c)
                if c
                in [
                    "gf_economic_capital_ead_amount",
                    "gf_ek_mitigated_el_adj_amount",
                    "gf_ek_adj_mit_dvrsfn_amount",
                ]
                else F.when(F.col(c) == 999.999999, F.lit(0))
                .otherwise(F.col(c))
                .alias(c)
                if c in ["gf_ek_aftr_mit_wght_pd_per"]
                else c
                for c in econ_capital_sel.columns
            ]
        )
        ...
        self.regl_capital_sel = regl_capital_sel.select(
            *[
                ...
                if c
                in [
                    "gf_rce_amd_exposure_amount",
                    "gf_rce_adm_mit_captl_amount",
                ...
                if c
                in [
                    "gf_aftr_mit_wght_pd_per",
                    "gf_rce_amd_appl_calc_lgd_per",
                    "gf_rw_sm_per",
                ]
                else c
                for c in regl_capital_sel.columns
            ]
        )
    ...
    def build_econ_reglty_capital(self):
        ...
        econ_capital_df = (
            self.fx.generic(
                self.econ_capital_sel.withColumn(
                    "g_currency_id",
                    F.when(F.col("g_entific_id").isin("MX"), F.lit("MXN")).when(
                        F.col("g_entific_id").isin("ES"), F.lit("EUR")
                    ),
                ),
                {
            ...
            .selectExpr(
                "g_contract_id",
                "gf_economic_capital_ead_amount",
                "gf_ek_aftr_mit_wght_pd_per",
                "gf_ek_mitigated_el_adj_amount",
                "gf_ek_adj_mit_dvrsfn_amount",
            )
            .join(self.contract_relations, on="g_contract_id", how="right")
        )

        # Regulatory amounts are cross-valued in local currency, in MXN for Mexico and in EUR for BBVA SA
        regl_capital_df = (
            self.fx.generic(
                self.regl_capital_sel.withColumn(
                    "g_currency_id",
                    F.when(F.col("g_entific_id").isin("MX"), F.lit("MXN")).when(
                        F.col("g_entific_id").isin("ES"), F.lit("EUR")
                    ),
        ...
        econ_capital_df = (
            ...
            .agg(
                ...
                F.max("gf_ek_mitigated_el_adj_amount").alias(
                    "gf_ek_mitigated_el_adj_amount"
                ),
                F.max("gf_ek_adj_mit_dvrsfn_amount").alias(
                    "gf_ek_adj_mit_dvrsfn_amount"
                ),
                (
                    F.sum(
                        F.col("gf_ek_aftr_mit_wght_pd_per")
                        * F.col("gf_economic_capital_ead_amount")
                    )
                    / F.sum("gf_economic_capital_ead_amount")
                ).alias("pd_m5_mitig_per"),
            )
            .join(
                self.rtg_score_letter_rel.selectExpr(
                    "gf_from_prblty_dflt_per",
                    "gf_to_prblty_dflt_per",
                    "g_smscl_internal_ratg_type AS  smscl_econ_internal_ratg_type",
                    "g_lmscl_internal_ratg_type AS lmscl_econ_internal_ratg_type",
                ),
                on=(
                    (F.col("gf_from_prblty_dflt_per") <= 100 * F.col("pd_m5_mitig_per"))
                    & (100 * F.col("pd_m5_mitig_per") < F.col("gf_to_prblty_dflt_per"))
                ),
                how="left",
            )
        ...
        regl_capital_df = (
            ...
            .agg(
                ...
                (
                    F.sum(
                        F.col("gf_aftr_mit_wght_pd_per")
                        * F.col("gf_rce_amd_exposure_amount")
                    )
                    / F.sum("gf_rce_amd_exposure_amount")
                ).alias("pd_ma_mitig_per"),
                (
                    F.sum(F.col("gf_rw_sm_per") * F.col("gf_rce_amd_exposure_amount"))
                    / F.sum("gf_rce_amd_exposure_amount")
                ).alias("gf_rw_sm_per"),
                F.max("gf_rce_amd_appl_calc_lgd_per").alias(
                    "gf_rce_amd_appl_calc_lgd_per"
                ),
            ...
            .join(
                ...
                on=(
                    (F.col("gf_from_prblty_dflt_per") <= 100 * F.col("pd_ma_mitig_per"))
                    & (100 * F.col("pd_ma_mitig_per") < F.col("gf_to_prblty_dflt_per"))
                ),
                how="left",
            )
            .drop("gf_from_prblty_dflt_per", "gf_to_prblty_dflt_per")
...
class IFRS9:
    ...
    def __init__(
        ...
        ifrs9 = (
            ...
            .get_ifrs9_rating(provision_type="N", entific_id=None)
    ...
    def build_ifrs9(self):
        ...
        ifrs9 = self.fx.generic(
            ...
            "entific_id",
            "final_stage_type",
            "final_provision_amount",
        )

        provisiones_df = (
            self.contract_relations.withColumn(
