...
class GuaranteesAndGuarantorsLoader:
    ...
    def get_guarantees_information(self, reference_date):
        ...
        guar_assignments = guarantees.get_guarantee_assignments(['ES', 'MX'],
                                                                from_date=reference_date,
                                                                to_date=reference_date)\
            .select('g_contract_id', 'g_guarantee_id', 'g_countervalued_currency_id', 'gf_ctvl_curr_guartd_amount',
                    'gf_beginning_guarantee_date', 'gf_end_guarantee_date', 'gf_cutoff_date')
        guar_struc_board = guarantees.get_guarantee_struc_board(['ES', 'MX'],
                                                                from_date=reference_date,
                                                                to_date=reference_date)\
            .select('g_guarantee_id', 'g_customer_id')

        if guar_assignments.count() == 0:
            self.logger.warning(f"No guarantee assignments found for the given date: {reference_date}")
            raise ValueError(f"No guarantee assignments found for the given date: {reference_date}")
        if guar_struc_board.count() == 0:
            self.logger.warning(f"No guarantee structure board data found for the given date: {reference_date}")
            raise ValueError(f"No guarantee structure board data found for the given date: {reference_date}")
...
class GuaranteesAndGuarantorsBuilder:
    ...
    def build_guaranteed_amounts(self):
        ...
        guar_assignments = ((
            ...
            ).filter((F.col('gf_beginning_guarantee_date') <= F.col('gf_cutoff_date')) &
        ...
        cust_bei = self.cust_data.filter(F.col('g_golden_customer_id') == 'A27721311')\
            .select('g_customer_id', F.lit(True).alias('BEI_flag'))
        guar_bei = self.guar_struc_board.join(cust_bei,
                                              on='g_customer_id', how='left')\
            .fillna(False, subset=['BEI_flag'])\
            .select('g_guarantee_id', 'BEI_flag')

        guar_amounts = (guar_assignments.join(guar_bei, on='g_guarantee_id', how='left')
                        .fillna(False, subset=['BEI_flag']))

        # Sum gf_ctvl_curr_guartd_amount and pivot depending on BEI_flag variable
        guar_amounts = guar_amounts.groupBy('g_contract_id')\
        ...
        guar_amounts = guar_amounts.join(self.contract_relations, on='g_contract_id', how='right')\
            .groupBy('delta_file_id', 'delta_file_band_id', 'branch_id')\
            .agg(F.sum('bei_guaranteed_amount').alias('bei_guaranteed_amount'),
