...
class FX:
    ...
    def generic(self, df, dict_cols_to_convert, preserve_cols=False, currency_id_name='currency_id'):
        """
        ...
        df = df. \
            ...
            fillna(1, subset=['fx'])
        df = df.select(*[(F.col('fx') * F.col(c)).alias(dict_cols_to_convert[c])
                         if c in dict_cols_to_convert else c for c in
                         df.columns] + (list(dict_cols_to_convert.keys()) if preserve_cols else []))
        return df
