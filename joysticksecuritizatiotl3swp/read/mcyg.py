...
from joysticksecuritizatiotl3swp.configurations.catalogues import tier_dict, segment_dict
...
class Maestro:  # pragma: no cover
    ...
    def __init__(self, logger, dataproc):
        ...
        self.c_cli = ['g_golden_customer_id',
                      ...
                      'gf_country_ifo_id',
                      'g_nace_activity_id']
        self.c_members = ['g_hier_level_type',
        ...
        self.c_hold_grupo = ['g_holding_group_id',
                             'g_country_id',
                             'gf_group_name',
                             'gf_tier_id',
    ...
    def read_mcyg_data(self, cli, cli_rel, members, hold_grupo):
        ...
        cli_rel = cli_rel.select('g_golden_customer_id', 'g_customer_id').dropDuplicates()
        ...
        members = members.where((F.col('g_hier_level_type').isin('CP', 'CC', 'SC')) &
        ...
        window_rel_ppal = W.partitionBy("g_customer_id").orderBy(F.col("gf_main_relation_group_type").asc())
        members = members. \
            withColumn('rank', F.rank().over(window_rel_ppal)). \
            filter(F.col('rank') == 1). \
            drop('rank')

        cli_final = (cli.join(cli_rel, on=['g_golden_customer_id'], how='inner')
                     .join(members, on=['g_customer_id'], how='inner'))
