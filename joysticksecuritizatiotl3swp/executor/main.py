...
class SecuritizationProcess:  # pragma: no cover
    ...
    def __init__(self, logger, spark, dataproc, parameters):
        ...
        if parameters["OUTPUT_MODE"] in ["DEVELOPMENT", "PRODUCTION"]:
            self.sandbox_path = self.constants.CONFIG[parameters["OUTPUT_MODE"]][
                "SANDBOX_PATH"
            ]
        else:
            raise ValueError(
                "Invalid output mode! Please use DEVELOPMENT or PRODUCTION."
            )

        # Load reference CLAN date
        if not re.fullmatch(r"[0-9]{8}|(\bNOT\s+AVAILABLE\b)", parameters["CLAN_DATE"]):
            raise ValueError(
                "Invalid CLAN_DATE format! Please use YYYYMMDD or NOT AVAILABLE."
            )
        ...
        if parameters["OUTPUT_MODE"] == "PRODUCTION":
            self.postgre_srvc = PostgreSQLService(
                db_name="dslb",
                execution_env="dataproc",
                host="-pgreporting.sandbox.extservices.platform.marathon.mss.",
                environ="live01",
                country="gl",
                port="5432"
            )
    ...
    def execute_process(self) -> int:
        ...
        _, date_econ = PartitionsUtils.get_newest_date_data(
            data=econ.filter(F.col("g_entific_id").isin(["ES", "MX"])),
        ...
        _, date_regl = PartitionsUtils.get_newest_date_data(
            data=regl.filter(F.col("g_entific_id").isin(["ES", "MX"])),
        ...
        if self.clan_date is not None:
            ...
            date_clan = max(
                PartitionsUtils.get_newest_date_data(
                    data=self.dataproc.read().parquet(self.paths.config_deals["path"]),
                    date_part_col=self.paths.config_deals["column_date"],
                )[1]
            ).replace("-", "")
        ...
        date_ifrs9 = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read()
                .parquet(self.paths.config_ifrs9["path"])
                .filter(F.col("provision_type") == "N")
                .filter(F.col("entific_id").isin("ES", "MX")),
                date_part_col=self.paths.config_ifrs9["column_date"],
            )[1]
        )
        ...
        date_maestro = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read().parquet(self.paths.path_members_rel),
                date_part_col=self.paths.campo_date,
            )[1]
        )
        ...
        date_cat = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read()
                .parquet(self.paths.path_rel_cat)
        ...
        date_ext_rating = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read()
                .parquet(self.paths.path_ext_rating)
        ...
        date_client_rat_attr = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read()
                .parquet(self.paths.path_cust_rating)
                .filter(F.col("g_entific_id") == "ES"),
                date_part_col=self.paths.campo_date,
            )[1]
        )
        self.logger.info(
            "Date Atributos ratings de clientes: " + str(date_client_rat_attr)
        )
        # Fecha Acreditadas
        date_accredited = max(
            PartitionsUtils.get_newest_date_data(
                data=self.dataproc.read().parquet(self.paths.path_acreditadas),
                date_part_col=self.paths.campo_date,
            )[1]
        )
        ...
        branches_df = (
            ...
            .withColumn(
                "rn",
                F.row_number().over(
                    W.partitionBy("branch_id", "entity_id").orderBy(
                        F.desc("entity_product")
                    )
                ),
            )
            .filter(F.col("rn") == 1)
        ...
        self.dslb_writer.write_df_to_sb(
            ...
            "contract_relations_securitizations",
            "parquet",
            "overwrite",
            10,
        )
        contract_relations = self.dataproc.read().parquet(
            f"{self.sandbox_path}auxiliar/contract_relations_securitizations"
        )
        ...
        econ_reglty_capital = EconRegltyCapital(
            self.logger,
            self.spark,
            self.dataproc,
            date_reg_econ_capital,
            fx,
            contract_relations,
        )
        (
            econ_capital_df,
            regl_capital_df,
        ) = econ_reglty_capital.build_econ_reglty_capital()

        # Info de IFRS9
        ifrs9_obj = IFRS9(
            self.logger, self.spark, self.dataproc, date_ifrs9, fx, contract_relations
        )
        ...
        ops_clan = (
            ...
            .fillna(
                {
                    "smscl_econ_internal_ratg_type": "No Rating",
        ...
        ops_clan = (
            ...
            .withColumn(
                "plazo_medio",
                F.datediff(F.to_date(F.col("vto_medio")), F.col("deal_signing_date"))
                / 365,
            )
        ...
        ops_clan = ops_clan.join(
            ifrs9_obj.get_cust_watchlist(), on="g_customer_id", how="left"
        ...
        cli = (
            self.dataproc.read()
            .parquet(self.paths.config_cli_ksag["path"])
            .filter(F.col(self.paths.config_cli_ksag["column_date"]) == date_maestro)
        )
        cli_rel = (
            self.dataproc.read()
            .parquet(self.paths.config_cli_rel["path"])
            .filter(F.col(self.paths.config_cli_rel["column_date"]) == date_maestro)
        )
        members = (
            self.dataproc.read()
            .parquet(self.paths.path_members_rel)
            .filter(F.col("gf_cutoff_date") == date_maestro)
        )
        hold_group = (
            self.dataproc.read()
            .parquet(self.paths.path_hold_group)
            .filter(F.col("gf_cutoff_date") == date_maestro)
        )
        ...
        ops_clan = ops_clan.join(
            maestro_grupos.select("g_customer_id", "g_holding_group_id"),
            on=["g_customer_id"],
            how="left",
        )
        ...
        clientes = (
            ...
            .drop_duplicates()
            .join(
                mcyg.get_cust_id_relation(date_maestro.replace("-", "")),
                on="g_golden_customer_id",
                how="left",
            )
        ...
        rel_actividad_aa = (
            rel_catalogos.where(
                (F.col("gf_initial_catalog_id") == "C039")
                & (F.col("gf_final_catalog_id") == "C164")
            )
            .select("gf_initial_catalog_val_id", "gf_final_catalog_val_id")
            .withColumnRenamed(
                "gf_initial_catalog_val_id", "gf_holding_activity_sector_id"
            )
            .withColumnRenamed(
                "gf_final_catalog_val_id", "g_asset_allocation_subsec_type"
            )
            .drop_duplicates()
        )
        rel_actividad_aa2 = (
        ...
        cat = (
            self.dataproc.read()
            .parquet(self.paths.path_cat)
            .filter(F.col("gf_frequency_type") == "M")
            .filter(F.col(self.paths.campo_date) == date_cat)
        )
        cat_subsector = (
            cat.where(F.col("g_catalog_id") == "C164")
            .select("gf_catalog_val_id", "gf_catlg_field_value_en_desc")
            .drop_duplicates()
            .withColumnRenamed("gf_catalog_val_id", "g_asset_allocation_subsec_type")
            .withColumnRenamed(
                "gf_catlg_field_value_en_desc", "g_asset_allocation_subsec_desc"
            )
        )
        cat_sector = (
            cat.where(F.col("g_catalog_id") == "C162")
            .select("gf_catalog_val_id", "gf_catlg_field_value_en_desc")
            .drop_duplicates()
            .withColumnRenamed("gf_catalog_val_id", "g_asset_class_id")
            .withColumnRenamed(
                "gf_catlg_field_value_en_desc", "g_asset_allocation_sector_desc"
            )
        )

        # SUBSECTOR Y SECTOR: Id's
        ops_clan = ops_clan.join(
            rel_actividad_aa, on=["gf_holding_activity_sector_id"], how="left"
        )
        ops_clan = ops_clan.join(
            rel_actividad_aa2, on=["gf_holding_activity_sector_id"], how="left"
        )
        # SUBSECTOR Y SECTOR: descripciones
        ops_clan = ops_clan.join(
            cat_subsector, on=["g_asset_allocation_subsec_type"], how="left"
        ).join(cat_sector, on=["g_asset_class_id"], how="left")

        countries_df = get_countries_iso_name_table(self.spark)
        ops_clan = (
            ops_clan.join(
                countries_df.withColumnRenamed(
                    "country_id", "group_country2"
            ...
            .join(
                countries_df.withColumnRenamed(
                    "country_id", "gf_country_ifo_id"
                ).withColumnRenamed("country_desc", "customer_country"),
                on=["gf_country_ifo_id"],
                how="left",
            )
            .join(
                countries_df.withColumnRenamed(
        ...
        grupo_cli_matriz = (
            maestro_grupos.where(F.col("gf_prtcpt_gr_header_type") == "S")
            .select("g_holding_group_id", "g_customer_id")
            .distinct()
            .join(
                cli_rel.select("g_golden_customer_id", "g_customer_id"),
                on=["g_customer_id"],
                how="left",
            )
        )
        ...
        rating_sp = (
            rating_sp.withColumn(
                "rn",
                F.row_number().over(
                    W.partitionBy("g_golden_customer_id").orderBy(
                        F.desc("g_sp_lt_rating_fc_type")
                    )
                ),
            )
            .filter("rn==1")
        ...
        rating_sp_matriz = (
            grupo_cli_matriz.drop("g_customer_id")
            .distinct()
            .join(rating_sp, on=["g_golden_customer_id"], how="inner")
        )
        ops_clan = ops_clan.join(
            rating_sp_matriz.drop("g_golden_customer_id"),
            on=["g_holding_group_id"],
            how="left",
        )
        ...
        t_kctk_accredited = (
            self.dataproc.read()
            .parquet(self.paths.path_acreditadas)
            .filter(F.col(self.paths.campo_date) == date_accredited)
            .withColumn(
                "rn",
                F.row_number().over(
                    W.partitionBy("gf_participant_id", "gf_pf_acct_fin_ent_id").orderBy(
                        F.desc("gf_pf_ratg_date"), F.desc("gf_contract_register_date")
                    )
                ),
            )
            .filter("rn==1")
            .select(
                F.when(
                    F.col("gf_pf_acct_fin_ent_id") == "0182",
                    F.concat(F.lit("ES0182"), F.col("gf_participant_id")),
                )
                .when(
                    F.col("gf_pf_acct_fin_ent_id") == "9016",
                    F.concat(F.lit("MX0074"), F.col("gf_participant_id")),
                )
                .alias("g_customer_id"),
        ...
        t_kctk_cust_rating_atrb = (
            self.dataproc.read()
            .parquet(self.paths.path_cust_rating)
            .filter(F.col(self.paths.campo_date) == date_client_rat_attr)
            .select(
                "g_customer_id",
        ...
        mvts = (
            ...
            .filter(
                (
                    F.col("gf_payment_situation_type_desc").isin(
                        "Manually Settled", "Settled"
                    )
                )
                & (
                    F.col("gf_movement_class_desc").isin(
                        "Fees", "Interest", "Fee Payment", "Repayment of principal"
                    )
                )
            )
            .select(
        ...
        ops_clan = ops_clan.withColumn(
            "sts_sm_rw_condition", 100 * F.col("gf_rw_sm_per") <= 100
        ...
        cubo_aud = (
            ops_clan.select(
                ...
                F.col("lmscl_reg_internal_ratg_type").alias(
                    "gf_ma_expanded_master_scale_id"
                ),
                "ma_expanded_master_scale_number",
                "pd_ma_mitig_per",
                F.col("gf_economic_capital_ead_amount").alias(
                    "gf_m5_economic_ead_amount"
                ),
            ...
            .withColumn(
                "EC_per",
                F.col("gf_m5_mitigation_rc_amount")
                / F.col("gf_m5_economic_ead_amount"),
            )
            .withColumn(
                "RC_per",
                F.col("gf_ma_mitigation_rc_amount") / F.col("gf_ma_ead_amount"),
            )
            .withColumn("Risk_Weight", 12.5 * F.col("RC_per"))
            .withColumn(
                "EL_per",
                F.col("gf_m5_total_el_amount") / F.col("gf_m5_economic_ead_amount"),
            )
            .withColumn(
                "Reg_EL_per",
                F.col("gf_ma_tot_expected_loss_amount") / F.col("gf_ma_ead_amount"),
            )
